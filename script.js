...
async function loadTypesInOverview(i) {
  ...
  for (let t = 0; t < pokemonTypes.length; t++) {
    ...
    container.innerHTML += `<div class="pokemon-type">${firstLetterUppercase(
      pokemonType
    )}</div>`;
  }
...
async function loadPokemonDetails(i) {
  ...
  document.getElementById("pokemon-name").innerHTML =
    firstLetterUppercase(pokemonName);
...
async function loadPokemonType(i) {
  ...
  for (let j = 0; j < pokemonTypes.length; j++) {
    ...
    ).innerHTML += `<div class="pokemon-type">${firstLetterUppercase(
      pokemonType
    )}</div>`;
  }
...
function setPokemonAbilities(abilities) {
  ...
  for (let a = 0; a < abilities.length; a++) {
    ...
    document.getElementById(
      "abilities"
    ).innerHTML += `<div>${firstLetterUppercase(ability)}</div>`;
  }
}
...
async function loadPokemonColor(i) {
  ...
  document.getElementById("color").innerHTML = `<div>${firstLetterUppercase(
    color
  )}</div>`;
}
...
function showLoadingOverlay() {
  ...
  document.getElementById("pokemon-search").setAttribute("readonly", "true");
}
...
function hideLoadingOverlay() {
  ...
  document
    ...
    .removeAttribute("readonly", "false");
}

async function prepareFieldForSearch() {
...
function toggleLoadMoreButton(action) {
  const button = document.getElementById('load-more-button');
  if (action === 'hide') button.style.display = 'none';
  if (action === 'show') button.style.display = 'inline-block';
}
...
function loadOVerviewCardsHTML(id, pokemonName) {
  return `<div id="overview-card-${id}" class="overview-card" onclick="loadPokemonDetails(${id})">
    ...
    <div class="overview-type-and-img-container">
        ...
        <img id="overview-image-${id}" alt="${firstLetterUppercase(
    pokemonName
  )}" title="${firstLetterUppercase(pokemonName)}">
    </div>
    </div>`;
}
